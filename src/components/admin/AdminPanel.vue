...
// Genera el ID completo desde el título: "ON-" + slug. Si ya existe otro
// cómic con ese ID, le añade -2, -3, … para que sea único.
function generatedId() {
  const slug = slugifyId(form.title)
  if (!slug) return ''
  const baseId = 'ON-' + slug
  let id = baseId
  let n = 2
...
async function save() {
  ...
  if (!newId) {
    error.value = 'El título es obligatorio (el ID se genera desde el título).'
    return
  }
  if (!form.title.trim()) {
  ...
  try {
    ...
    if (idChanged) {
      try {
        ...
        throw new Error(
          `Nuevo cómic guardado como "${newId}", pero no se pudo eliminar el antiguo "${editingId.value}": ${e.message}`
        )
      }
    }
    await load()
...
async function processCover(file) {
  error.value = ''
  if (!form.title.trim()) {
    error.value = 'Primero escribe el título del cómic.'
    return
  }
      ...
      <div v-else class="mt-6 divide-y divide-white/10 border-y border-white/10">
        <div v-for="comic in comics" :key="comic.id" class="flex flex-col gap-3 py-4 md:flex-row md:items-center md:justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-[10px] font-semibold uppercase tracking-[0.3em] text-white/40">{{ formatIssue(comic.issue) }} — {{ comic.year }}</p>
      ...
      <Teleport to="body">
        <Transition name="admin-modal">
          <div
            ...
            role="dialog"
            aria-modal="true"
            :aria-label="editingId ? 'Editar cómic' : 'Nuevo cómic'"
          >
              ...
              <div class="px-6 py-6 md:px-8">
                <div class="grid gap-6 md:grid-cols-3">
                  <p class="text-[10px] uppercase tracking-[0.3em] text-white/40 md:col-span-3">
                    ID del producto (se genera automáticamente): <span class="font-bold text-white">{{ generatedId() || '…' }}</span>
                  </p>
                  ...
                  <label class="block text-[10px] font-bold uppercase tracking-[0.3em] text-white/50">
                    Año (detecta el actual, editable)
                    <input
