...
<template>
  <div class="fixed inset-0 z-[90] overflow-y-auto bg-ink-950/95 backdrop-blur-sm" @click.self="emit('close')">
    <div class="mx-auto flex min-h-full max-w-5xl items-center justify-center p-4 md:p-8">
      <!-- Cargando -->
      <div v-if="loading" class="w-full animate-pulse border border-ink-700">
      ...
      <!-- No encontrado -->
      <div v-else-if="!comic" class="w-full max-w-md border border-ink-700 p-10 text-center">
      ...
      <!-- Comic sheet -->
      <div v-else class="relative w-full border border-ink-700 bg-ink-950">
        <button
          @click="emit('close')"
        ...
        <div class="grid md:grid-cols-[minmax(280px,380px)_1fr]">
          <!-- Portada -->
          <div class="relative overflow-hidden border-b border-ink-700 md:border-b-0 md:border-r">
            <img
          ...
          <div class="flex flex-col p-6 md:p-10">
            <div class="flex flex-col items-center text-center">
              <p class="text-[10px] font-bold uppercase tracking-[0.4em] text-ink-400">
                {{ comic.year }}
              </p>
              <h2 class="font-display mt-2 text-3xl font-black uppercase leading-[0.95] tracking-tight text-white sm:text-4xl">
                {{ comic.title }}
              </h2>
              <p class="mt-2 text-base font-semibold uppercase tracking-[0.2em] text-ink-300">Issue {{ comic.issue }}</p>
            </div>

            <!-- Metadatos -->
            <div class="mt-8 flex justify-center">
              <dl class="grid grid-cols-2 gap-px border border-ink-700 bg-ink-700 text-center">
                <div class="bg-ink-900 p-3 md:p-4">
                  <dt class="text-[10px] font-bold uppercase tracking-[0.25em] text-ink-400">Pages</dt>
                  <dd class="font-display mt-1 text-lg font-black text-ink-100">{{ comic.pages || '—' }}</dd>
                </div>
                <div class="bg-ink-900 p-3 md:p-4">
                  <dt class="text-[10px] font-bold uppercase tracking-[0.25em] text-ink-400">Author</dt>
                  <dd class="font-display mt-1 min-w-0 break-words text-lg font-black text-ink-100">{{ comic.author }}</dd>
                </div>
              </dl>
            </div>

            <!-- Compartir -->
            <div class="mt-6 border border-ink-700 bg-ink-900/30 p-4">
              <p class="text-[10px] font-bold uppercase tracking-[0.3em] text-ink-400">Share</p>
        ...
        <div v-if="related.length" class="border-t border-ink-700 px-6 py-8 md:px-10">
          <p class="text-[10px] font-bold uppercase tracking-[0.4em] text-ink-400">More from this series</p>
          ...
          <div class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-5 lg:grid-cols-4">
            <a v-for="r in related" :key="r.id" :href="`/comics/${r.slug}`" class="group block">
              ...
              <p class="mt-3 text-[10px] font-semibold uppercase tracking-[0.3em] text-ink-400">{{ r.year }}</p>
              <h4 class="font-display mt-1 text-sm font-black uppercase tracking-tight text-white">{{ r.title }}</h4>
              <p class="mt-0.5 text-xs text-ink-300">Issue {{ r.issue }}</p>
            </a>
          </div>
        </div>
        ...
        <p
          ...
          class="border-t border-ink-700 bg-ink-900/30 px-6 py-8 text-sm leading-relaxed text-ink-200 md:px-10"
        >
          {{ comic.description }}
        </p>
      </div>
